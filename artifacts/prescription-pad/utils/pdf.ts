...
export function prescriptionHtml(doctor: DoctorProfile, draft: PrescriptionDraft) {
  const medicines = draft.medicines
    ...
    .join('');
  const logo = doctor.logoUri ? `<img class="logo" src="${doctor.logoUri}" />` : '';
  ...
  * { box-sizing: border-box; } body { font-family: Arial, sans-serif; color:#17212B; font-size:10pt; margin:0; }
  ...
  .signature { margin-top:88px; text-align:right; } .signature-line { display:inline-block; width:140px; border-top:1px solid #17212B; } .signature-label { display:block; color:#657476; font-size:8pt; margin-top:5px; }
  footer { position:fixed; bottom:0; left:0; right:0; color:#657476; text-align:center; font-size:7pt; }
  </style></head><body>
  ...
  <div class="signature"><span class="signature-line"></span><span class="signature-label">Doctor signature</span></div><footer>${escapeHtml(doctor.footerText)}</footer>
  </body></html>`;
}

export async function generatePrescriptionPdf(doctor: DoctorProfile, draft: PrescriptionDraft) {
  const { uri } = await Print.printToFileAsync({ html: prescriptionHtml(doctor, draft), base64: false });
  return uri;
}
