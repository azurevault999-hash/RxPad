...
import {
  Alert, Keyboard, Modal, Pressable, ScrollView, StyleSheet, Text, TextInput, View,
} from 'react-native';
import { Feather } from '@expo/vector-icons';
import * as Haptics from 'expo-haptics';
import { KeyboardAwareScrollViewCompat } from '@/components/KeyboardAwareScrollViewCompat';
import { A4Preview } from '@/components/A4Preview';
import { MEDICINE_SOURCE_LABEL, medicineProvider } from '@/data/medicineProvider';
import { useColors } from '@/hooks/useColors';
import { usePrescription } from '@/state/PrescriptionContext';
import { PrescriptionMedicine, Sex } from '@/types/prescription';
import { generatePrescriptionPdf, printPdf, savePdf, sharePdf } from '@/utils/pdf';

type FieldProps = { label: string; value: string; onChangeText: (text: string) => void; placeholder?: string; multiline?: boolean; keyboardType?: 'default' | 'numeric' };
...
export default function WritePrescriptionScreen() {
  ...
  const [selectedMedicine, setSelectedMedicine] = useState<PrescriptionMedicine | null>(null);
  const [showMore, setShowMore] = useState(false);
  const [showPreview, setShowPreview] = useState(false);
  const [busy, setBusy] = useState(false);
  const suggestions = useMemo(() => medicineProvider.search(query), [query]);

  const update = <K extends keyof typeof draft>(key: K, value: (typeof draft)[K]) => setDraft((current) => ({ ...current, [key]: value }));
  const selectMedicine = (medicine: PrescriptionMedicine | null) => {
    setSelectedMedicine(medicine ?? {
      ...suggestions[0],
      dose: '1 tablet',
      frequency: 'TDS',
      duration: '5 days',
      prescribedRoute: suggestions[0]?.route ?? 'Oral',
      instructions: 'After food',
    });
    setQuery('');
  };
  const updateSelected = <K extends keyof PrescriptionMedicine>(key: K, value: PrescriptionMedicine[K]) => setSelectedMedicine((current) => current ? { ...current, [key]: value } : current);
  const addMedicine = () => {
    if (!selectedMedicine) return;
    setDraft((current) => ({ ...current, medicines: [...current.medicines, selectedMedicine] }));
    ...
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      <KeyboardAwareScrollViewCompat contentContainerStyle={styles.content} bottomOffset={24} keyboardShouldPersistTaps="handled">
        <View style={styles.topRow}>
          ...
          <Pressable onPress={() => { resetDraft(); setSelectedMedicine(null); }} style={[styles.iconButton, { backgroundColor: colors.card, borderColor: colors.border }]}><Feather name="plus" size={20} color={colors.primary} /></Pressable>
        </View>
        ...
        <View style={[styles.sourcePill, { backgroundColor: colors.secondary }]}><Feather name="database" size={13} color={colors.primary} /><Text style={[styles.sourceText, { color: colors.secondaryForeground }]}>{MEDICINE_SOURCE_LABEL}</Text></View>
        <View style={[styles.searchBox, { backgroundColor: colors.card, borderColor: colors.border }]}><Feather name="search" size={18} color={colors.mutedForeground} /><TextInput value={query} onChangeText={setQuery} placeholder="Search medicine, composition or strength" placeholderTextColor={colors.mutedForeground} style={[styles.searchInput, { color: colors.foreground }]} onFocus={() => setQuery('')} /></View>
        {query.length > 0 ? <View style={[styles.suggestions, { backgroundColor: colors.card, borderColor: colors.border }]}>{suggestions.map((medicine) => <Pressable key={medicine.id} onPress={() => selectMedicine({ ...medicine, dose: '1 tablet', frequency: 'TDS', duration: '5 days', prescribedRoute: medicine.route, instructions: 'After food' })} style={styles.suggestion}><View style={styles.suggestionCopy}><Text style={[styles.suggestionName, { color: colors.foreground }]}>{medicine.productName}</Text><Text style={[styles.suggestionMeta, { color: colors.mutedForeground }]}>{medicine.composition}</Text></View><Feather name="arrow-up-left" size={16} color={colors.primary} /></Pressable>)}</View> : null}
        {selectedMedicine ? <View style={[styles.medicineEditor, { backgroundColor: colors.secondary }]}><View style={styles.editorTitleRow}><View style={{ flex: 1 }}><Text style={[styles.editorName, { color: colors.foreground }]}>{selectedMedicine.productName}</Text><Text style={[styles.suggestionMeta, { color: colors.mutedForeground }]}>{selectedMedicine.composition}</Text></View><Pressable onPress={() => setSelectedMedicine(null)}><Feather name="x" size={19} color={colors.mutedForeground} /></Pressable></View><View style={styles.row}><View style={styles.half}><Field label="Dose" value={selectedMedicine.dose} onChangeText={(value) => updateSelected('dose', value)} placeholder="1 tablet" /></View><View style={styles.half}><Field label="Frequency" value={selectedMedicine.frequency} onChangeText={(value) => updateSelected('frequency', value)} placeholder="TDS" /></View></View><View style={styles.row}><View style={styles.half}><Field label="Duration" value={selectedMedicine.duration} onChangeText={(value) => updateSelected('duration', value)} placeholder="5 days" /></View><View style={styles.half}><Field label="Route" value={selectedMedicine.prescribedRoute} onChangeText={(value) => updateSelected('prescribedRoute', value)} placeholder="Oral" /></View></View><Field label="Instructions" value={selectedMedicine.instructions} onChangeText={(value) => updateSelected('instructions', value)} placeholder="After food" /><Pressable onPress={addMedicine} style={[styles.addButton, { backgroundColor: colors.primary }]}><Feather name="plus" size={18} color={colors.primaryForeground} /><Text style={[styles.addButtonText, { color: colors.primaryForeground }]}>Add to prescription</Text></Pressable></View> : null}
        {draft.medicines.map((medicine, index) => <View key={`${medicine.id}-${index}`} style={[styles.medicineCard, { backgroundColor: colors.card, borderColor: colors.border }]}><View style={styles.medicineCardTop}><View style={{ flex: 1 }}><Text style={[styles.medicineCardName, { color: colors.foreground }]}>{medicine.productName}</Text><Text style={[styles.suggestionMeta, { color: colors.mutedForeground }]}>{medicine.composition}</Text></View><Pressable onPress={() => setDraft((current) => ({ ...current, medicines: current.medicines.filter((_, itemIndex) => itemIndex !== index) }))}><Feather name="trash-2" size={17} color={colors.destructive} /></Pressable></View><Text style={[styles.medicineSummary, { color: colors.primary }]}>{medicine.dose}  ·  {medicine.frequency}  ·  {medicine.duration}  ·  {medicine.prescribedRoute}</Text><Text style={[styles.suggestionMeta, { color: colors.mutedForeground }]}>{medicine.instructions}</Text><View style={styles.cardActions}><Pressable onPress={() => selectMedicine(medicine)}><Text style={[styles.actionText, { color: colors.primary }]}>Edit</Text></Pressable><Pressable onPress={() => setDraft((current) => ({ ...current, medicines: [...current.medicines.slice(0, index + 1), { ...medicine, id: `${medicine.id}-copy-${Date.now()}` }, ...current.medicines.slice(index + 1)] }))}><Text style={[styles.actionText, { color: colors.primary }]}>Duplicate</Text></Pressable><Pressable disabled={index === 0} onPress={() => setDraft((current) => { const medicines = [...current.medicines]; [medicines[index - 1], medicines[index]] = [medicines[index], medicines[index - 1]]; return { ...current, medicines }; })}><Feather name="chevron-up" size={17} color={index === 0 ? colors.border : colors.primary} /></Pressable><Pressable disabled={index === draft.medicines.length - 1} onPress={() => setDraft((current) => { const medicines = [...current.medicines]; [medicines[index], medicines[index + 1]] = [medicines[index + 1], medicines[index]]; return { ...current, medicines }; })}><Feather name="chevron-down" size={17} color={index === draft.medicines.length - 1 ? colors.border : colors.primary} /></Pressable></View></View>)}
...
const styles = StyleSheet.create({
  ...
  sourcePill: { flexDirection: 'row', alignSelf: 'flex-start', alignItems: 'center', gap: 6, paddingHorizontal: 9, paddingVertical: 6, borderRadius: 20, marginBottom: 9 },
  sourceText: { fontSize: 11, fontWeight: '600' },
  searchBox: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderRadius: 13, paddingHorizontal: 13, minHeight: 50 },
  searchInput: { flex: 1, paddingHorizontal: 10, fontSize: 14 },
  suggestions: { borderWidth: 1, borderRadius: 13, marginTop: 5, overflow: 'hidden' },
  suggestion: { flexDirection: 'row', alignItems: 'center', gap: 10, padding: 13, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#D8E2E0' },
  suggestionCopy: { flex: 1 },
  suggestionName: { fontSize: 14, fontWeight: '600' },
  suggestionMeta: { fontSize: 11, marginTop: 3, lineHeight: 15 },
  medicineEditor: { marginTop: 12, padding: 14, borderRadius: 16 },
