...
export default function SettingsScreen() {
  ...
  return <ScrollView style={[styles.screen, { backgroundColor: colors.background }]} contentContainerStyle={styles.content}>
    ...
    <Pressable onPress={() => pickImage('logoUri')} style={[styles.logoPicker, { backgroundColor: colors.card, borderColor: colors.border }]}>{form.logoUri ? <Image source={{ uri: form.logoUri }} style={styles.logo} /> : <View style={[styles.logoPlaceholder, { backgroundColor: colors.secondary }]}><Feather name="image" size={22} color={colors.primary} /></View>}<View style={{ flex: 1 }}><Text style={[styles.logoTitle, { color: colors.foreground }]}>{form.logoUri ? 'Change clinic logo' : 'Add clinic logo'}</Text><Text style={[styles.logoHint, { color: colors.mutedForeground }]}>Optional · appears in the header</Text></View><Feather name="chevron-right" size={18} color={colors.mutedForeground} /></Pressable>
    <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Prescription assets</Text>
    <Pressable onPress={() => pickImage('signatureImageUri')} style={[styles.assetPicker, { backgroundColor: colors.card, borderColor: colors.border }]}>
      {form.signatureImageUri ? <Image source={{ uri: form.signatureImageUri }} style={styles.assetPreview} resizeMode="contain" /> : <View style={[styles.assetPlaceholder, { backgroundColor: colors.secondary }]}><Feather name="edit-3" size={22} color={colors.primary} /></View>}
      <View style={{ flex: 1 }}><Text style={[styles.logoTitle, { color: colors.foreground }]}>{form.signatureImageUri ? 'Change signature' : 'Add signature'}</Text><Text style={[styles.logoHint, { color: colors.mutedForeground }]}>Optional · appears above the signature line</Text></View>
      <Feather name="chevron-right" size={18} color={colors.mutedForeground} />
    </Pressable>
    {form.signatureImageUri ? <Pressable onPress={() => update('signatureImageUri', '')} style={styles.removeButton}><Text style={[styles.removeText, { color: colors.destructive }]}>Remove signature</Text></Pressable> : null}
    <Pressable onPress={() => pickImage('qrCodeImageUri')} style={[styles.assetPicker, { backgroundColor: colors.card, borderColor: colors.border }]}>
      {form.qrCodeImageUri ? <Image source={{ uri: form.qrCodeImageUri }} style={styles.assetPreview} resizeMode="contain" /> : <View style={[styles.assetPlaceholder, { backgroundColor: colors.secondary }]}><Feather name="grid" size={22} color={colors.primary} /></View>}
      <View style={{ flex: 1 }}><Text style={[styles.logoTitle, { color: colors.foreground }]}>{form.qrCodeImageUri ? 'Change QR code' : 'Add QR code'}</Text><Text style={[styles.logoHint, { color: colors.mutedForeground }]}>Optional · upload an existing QR image</Text></View>
      <Feather name="chevron-right" size={18} color={colors.mutedForeground} />
    </Pressable>
    {form.qrCodeImageUri ? <Pressable onPress={() => update('qrCodeImageUri', '')} style={styles.removeButton}><Text style={[styles.removeText, { color: colors.destructive }]}>Remove QR code</Text></Pressable> : null}
...
const styles = StyleSheet.create({
  ...
  assetPicker: { borderWidth: 1, borderRadius: 16, padding: 13, flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 6 },
  assetPreview: { width: 52, height: 52, borderRadius: 13 },
  assetPlaceholder: { width: 52, height: 52, borderRadius: 13, alignItems: 'center', justifyContent: 'center' },
  removeButton: { alignSelf: 'flex-end', paddingHorizontal: 4, paddingBottom: 14 },
